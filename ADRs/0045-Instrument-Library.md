...
| `:sort-order` | integer | always | Controls display order within a `:family` group. Bundled entries use multiples of 1000 (score order: `1000`, `2000`, `3000`, …). Drag-to-reorder assigns the midpoint between neighbours; if the sum of the two neighbours is odd (i.e. the exact midpoint would not be an integer), the frontend renumbers the entire family at 1000-spacing before computing the midpoint. User-added instruments default to `(max sort-order in family) + 1000`. |
...
| `:family` | keyword | always | Instrument family: `:woodwind`, `:brass`, `:strings`, `:percussion`, `:keyboard`, `:plucked`, `:voice`, `:other` |
...
Voice ▶, Other ▶. Within each family, instruments are sorted ascending by `:sort-order`. Instrument names
...
instruments B and C assigns A a new `:sort-order` of `(sort-order(B) + sort-order(C)) / 2`. If the sum of B and C's sort-orders is odd (midpoint would not be an integer), the frontend
renumbers the entire family at 1000-spacing before computing the midpoint. The reordered instruments vector is then submitted via
`set-instrument-library`.
