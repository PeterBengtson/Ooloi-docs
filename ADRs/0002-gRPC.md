...
- [Rationale](#rationale)
  ...
  - [Java Interop Advantages](#java-interop-advantages-updated-july-2025)
...
Ooloi leverages gRPC's comprehensive streaming capabilities to enable real-time collaborative music notation:
...
2. **Server Streaming** (Request-Stream Response):
   - **Real-time collaboration events**: Backend pushes piece changes to all subscribed clients
   - **Progress updates**: Long-running operations (MIDI generation, complex layout calculations)
   - **Live synchronization**: Multiple musicians editing the same piece simultaneously
   - **Graphics recomputation**: Backend recalculates and streams updated drawing instructions
...
4. **Bidirectional Streaming** (Stream Request-Stream Response):
   - **Interactive collaboration**: Real-time editing with immediate feedback between multiple users
...
### Universal Clojure-Aware gRPC Architecture (Updated 2025)

**Breaking Change**: The implementation approach has been fundamentally revised to use a **universal Clojure-aware protobuf schema** that eliminates complex API introspection and reflection-based code generation. This approach enables **hot plugin installation** and dramatically simplifies the gRPC layer.

### 1. Universal Protobuf Schema

Replace domain-specific protobuf messages with a single universal schema:
...
// Universal Clojure value representation
message ClojureValue {
  oneof value_type {
    string string_val = 1;
    int64 int_val = 2;
    double double_val = 3;
...
message OoloiRequest {
  string method = 1;
  ClojureValue params = 2;
}

message OoloiResponse {
  bool success = 1;
  optional string error = 2;
  optional ClojureValue result = 3;
}
...
(ns ooloi.backend.grpc.server
  (:require [ooloi.backend.grpc.clojure-conversion :as conv]
            [ooloi.backend.api :as api])
  (:import [io.grpc Status]
           [io.grpc.stub StreamObserver]))

(defn execute-method
  "Universal method executor for any API call."
...
- **No schema regeneration**: Plugins define new models → universal ClojureValue handles them automatically
...
2. Universal ClojureValue handles all new data types immediately  
