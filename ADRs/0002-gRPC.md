...
- [Implementation Approach](#implementation-approach)
  - [Unified Clojure-Aware gRPC Architecture](#unified-clojure-aware-grpc-architecture)
  - [1. Unified Protobuf Schema](#1-unified-protobuf-schema)
  - [2. Dependencies and Build Configuration](#2-dependencies-and-build-configuration)
  - [3. Simplified Server Implementation](#3-simplified-server-implementation)
  - [4. Hot Plugin Installation Benefits](#4-hot-plugin-installation-benefits)
  - [2. Define Protocol Buffer Messages and Services](#2-define-protocol-buffer-messages-and-services)
  - [3. Code Generation Pipeline](#3-code-generation-pipeline)
  - [4. Backend gRPC Server Implementation](#4-backend-grpc-server-implementation)
  - [5. Architectural Separation](#5-architectural-separation)
  - [6. Backend Client Registration and Streaming Implementation](#6-backend-client-registration-and-streaming-implementation)
  - [7. Error Handling and Retry Mechanisms](#7-error-handling-and-retry-mechanisms)
  - [8. Testing Infrastructure](#8-testing-infrastructure)
  - [9. Security Implementation](#9-security-implementation)
  - [10. System Startup and Communication Flow](#10-system-startup-and-communication-flow)
  - [11. Monitoring and Observability](#11-monitoring-and-observability)
- [Alternatives Considered](#alternatives-considered)
  - [Port Number Alternatives Considered](#port-number-alternatives-considered)
  - [TLS Implementation Alternatives Considered](#tls-implementation-alternatives-considered)
...
Ooloi's architecture supports **two deployment models**: a combined desktop application (frontend and backend in a single JVM) and a backend-only server for dedicated deployments. The **backend hosts a gRPC server** while the **frontend acts as a gRPC client**. In combined deployments, both client and server run in the same process using in-process transport, with the ability to dynamically enable network access for collaboration (see [ADR-0036](0036-Collaborative-Sessions-and-Hybrid-Transport.md)). We need to choose a protocol that can handle complex data structures, provide type safety, and offer high performance for real-time interactions with large musical scores.
...
We will use **gRPC with Java interop** (grpc-java) for communication between Ooloi's frontend and backend components. The **shared project serves dual purposes**: containing protocol definitions and shared utilities, plus packaging the combined desktop application, supporting two deployment models:

1. **Combined** binary (both client and server in same process, built from `shared/`)
2. **Backend-only** binary (server only, built from `backend/`)

In all cases, the backend component hosts the gRPC server while the frontend component acts as a gRPC client. The frontend project is a code library consumed by the combined application; it does not produce a standalone artifact.
...
3. **Client Streaming** (Stream Request-Response) — **future**:
   - No client-streaming RPC is currently used. Atomic multi-operation batches ride the unary `ExecuteMethod` (via `SRV/atomic`): the ACID guarantee is the backend's single STM transaction (`execute-atomic-operations`' `dosync`), a handler-level guarantee — gRPC has no transaction semantics of its own — so no streaming RPC is needed for atomicity.
   - A client-streaming RPC would earn its place only for **large-body** transport — chunking a very large MusicXML/MIDI import so it need not arrive as one message (size ceiling, backpressure). That, and bulk file transfer, are streaming concerns that will get their **own ADR** when a concrete consumer (e.g. MusicXML import) arrives.
...
- **Deployment flexibility** supporting combined desktop application or backend-only server deployments
...
- **Non-VPD singleton API functions**: The universal `handle-execute-method` dispatch (using `ns-resolve` to find API functions at call time) enables a clean pattern for singleton operations that do not require piece-path navigation. Functions declared with `^{:api true}` but without `:vpd-category` in `interfaces.clj` dispatch to a fixed keyword and are implemented in `shared/ops/` namespaces. Because shared code cannot require backend namespaces directly, implementations access backend component state via `resolve` on the single `*server-component*` dynamic var defined in `grpc/server.clj` and bound per-request by the handler; component deps appear as keys on that map. The frontend calls these functions as `SRV/*` through the existing transport with no new protobuf required. See [POLYMORPHIC_API_GUIDE](../guides/POLYMORPHIC_API_GUIDE.md) for the full pattern description and the error-surfacing contract.
...
### Unified Clojure-Aware gRPC Architecture
...
**Shared project** (`shared/src/main/proto/ooloi_service.proto`):
```protobuf
syntax = "proto3";
package ooloi;

// Unified Clojure value representation. Every variant in the oneof represents
// a distinct Clojure type. The `nil_val` variant is essential — without it,
// nil entries inside repeated OoloiValue fields (vector elements, map values,
// set elements) would be silently dropped by the protobuf runtime.
message OoloiValue {
  oneof value_type {
    string string_val = 1;
    int64 int_val = 2;
    double double_val = 3;
    bool bool_val = 4;
    OoloiRatio ratio_val = 5;
    OoloiKeyword keyword_val = 6;
    OoloiSymbol symbol_val = 7;
    OoloiMap map_val = 8;
    OoloiVector vector_val = 9;
    OoloiSet set_val = 10;
    OoloiList list_val = 11;
    OoloiDefrecord defrecord_val = 12;
    bool nil_val = 13;  // Explicit nil — preserves nil through repeated fields.
  }
}
...
message OoloiKeyword {
  string namespace = 1;  // Empty string for no namespace.
  string name = 2;
}

// Additional supporting message types (OoloiSymbol, OoloiMap, OoloiVector,
// OoloiSet, OoloiList, OoloiDefrecord) follow the same pattern.
```
...
**Shared project** (`shared/project.clj`). The versions are held in `project.clj` and kept identical across all three projects; the compiler and the gRPC plugin are pinned to match the runtimes they generate against.
...
Per [ADR-0046](0046-Reference-Passing-In-Process-Transport.md), the wire ↔ Clojure conversion is encapsulated entirely in transport-aware marshallers (`shared/grpc/transport.clj`, `shared/grpc/service_builder.clj`). Handler functions receive and return native Clojure maps regardless of transport — they have no `import` of any protobuf class and no per-call conversion logic. There is one live application handler, for the unary `ExecuteMethod` RPC:

```clojure
(ns ooloi.backend.grpc.server
  (:require [ooloi.shared.grpc.headers :as headers]
            [ooloi.backend.grpc.stats :as stats]))

;; *server-component* is the only dynamic var. Backend Integrant deps injected
;; into the gRPC server (e.g. :instrument-library-component, :undo-manager-component)
;; are read off this map. See POLYMORPHIC_API_GUIDE for the canonical pattern.
(def ^:dynamic *server-component* nil)

(defn- handle-execute-method
  "Handler for ExecuteMethod unary RPC. Receives and returns native Clojure maps.
   Returns {:success bool :error str :result <native-clojure-value>}."
  [request server-component]
  (let [client-id     (headers/get-client-id-from-context)
        request-bytes (transport/request-serialized-size request)
        start-nanos   (System/nanoTime)]
    (try
      (let [method-name (:method request)
            api-fn      (resolve-api-var method-name)
            result      (if api-fn
                          (binding [*server-component* server-component]
                            {:success true
                             :error   ""
                             :result  (invoke-with-params api-fn (:params request))})
                          {:success false
                           :error   (str "Method not found: " method-name)
                           :result  nil})]
        (stats/increment-api-stats server-component client-id
                                   {:success?       (:success result)
                                    :bytes-received request-bytes
                                    :duration-nanos (- (System/nanoTime) start-nanos)})
        result)
      (catch Throwable e
        ;; Errors as values, not exceptions. The SRV/* client wrapper inspects
        ;; :success false and throws ex-info with the server's exact :error
        ;; message — see POLYMORPHIC_API_GUIDE §"Error Surfacing Contract".
        (stats/increment-api-stats server-component client-id
                                   {:success? false
                                    :bytes-received request-bytes
                                    :duration-nanos (- (System/nanoTime) start-nanos)})
        (stats/increment-error-stats server-component client-id
                                     {:error-type (stats/categorize-exception e)})
        {:success false
         :error   (str "Execution error: " (.getMessage e))
         :result  nil}))))
```

The error path returns a response map rather than letting the exception propagate. The full rationale (error-message fidelity, inspectable-as-data testing, stats integrity, one contract across all transport paths) is documented in [POLYMORPHIC_API_GUIDE §Error Surfacing Contract](../guides/POLYMORPHIC_API_GUIDE.md#error-surfacing-contract-response-data-client-side-throw).
...
One naming requirement attaches to that automatic handling: a Clojure plugin's namespaces must be
named with hyphens rather than literal underscores, because a defrecord travels as its *munged*
Java class name and reconstruction has to demunge it back into a namespace. See
[ADR-0003: Plugins §Namespace Naming Requirement for Plugin Defrecords](0003-Plugins.md#namespace-naming-requirement-for-plugin-defrecords)
for the reason and its scope — it applies to Clojure defrecords only, since plugins written in
other JVM languages send maps, which need no namespace resolution at all.
...
- **VPD utilities** (`shared/ops/vpd.clj`) - Both sides manipulate Vector Path Descriptors
- **Protocol conversion** (`shared/grpc/clojure_conversion.clj`, `shared/grpc/protobuf_bridge.clj`) - Bidirectional Clojure ↔ Protocol Buffer conversion, split context-free and transport-specific
- **Musical vocabulary** (`shared/ops/pitches.clj`, `shared/ops/accidental_maps.clj`, `shared/hierarchy.clj`) - Pitch parsing, letter-aware accidental maps, and the type hierarchy both sides dispatch on
- **Build utilities** (`shared/build.clj`) - Cross-project packaging tools
...
This dual-purpose architecture enables Ooloi's **two deployment modes** (combined desktop application, backend-only server) while maximizing code reuse and maintaining type safety. See [ADR-0036](0036-Collaborative-Sessions-and-Hybrid-Transport.md) for the hybrid transport architecture that allows the combined application to dynamically enable network access for collaboration.
...
Ooloi supports both IPv4 and IPv6 throughout. The gRPC client builds its channel with `NettyChannelBuilder/forAddress(host, port)`, passing host and port **separately** — so the host may be a hostname, an IPv4 dotted-quad, or an **IPv6 literal** (e.g. `::1`, `2001:db8::1`, `fe80::1`), with no bracketing required (the `[host]:port` bracket form is only a `forTarget` URI concern, which Ooloi does not use). The "Connect to other Ooloi…" dialog and the `OOLOI_*` / `OOLOI_FRONTEND_*` host configuration accept all three forms; the connect-host validator admits hostnames, IPv4, and IPv6 literals and rejects only malformed input (URL schemes such as `https://`, embedded ports, spaces). The auto-generated development certificate's SANs cover `localhost`, `127.0.0.1`, and `::1`, so local TLS works over both families.
...
**Deployment Model 1: Combined Desktop Application** (primary)
1. Single application starts with both backend and frontend components
2. Backend component starts with in-process gRPC transport (zero network overhead)
3. Frontend component initializes gRPC client components using in-process transport
4. Application logic calls `register-with-server` when ready to establish connections (e.g., after UI initialization)
5. All communication happens in-process with integrated streaming
6. Can dynamically enable network server for collaboration sessions (see [ADR-0036](0036-Collaborative-Sessions-and-Hybrid-Transport.md))

**Deployment Model 2: Backend-only**
1. Backend binary starts with gRPC server on configured network port
2. External clients (other combined applications as guests, future web frontend, mobile apps, etc.) connect over network
3. Client applications call connection establishment methods when ready to connect
4. Useful for dedicated servers, headless servers, or API-only deployments
...
- [ADR-0046: Reference-Passing In-Process Transport](0046-Reference-Passing-In-Process-Transport.md) - Reference-passing marshallers eliminating Clojure↔protobuf conversion for in-process transport
