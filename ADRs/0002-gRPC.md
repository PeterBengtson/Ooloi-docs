...
3. Frontend component registers as gRPC client to the same process
4. All communication happens in-process or via localhost with integrated streaming
...
3. Frontend gRPC client registers with backend server over network
4. Frontend registration establishes both API capability and event streaming for all musical operations
