...
- **Non-VPD singleton API functions**: The universal `execute-method` dispatch (using `ns-resolve` to find API functions at call time) enables a clean pattern for singleton operations that do not require piece-path navigation. Functions declared with `^{:api true}` but without `:vpd-category` in `interfaces.clj` dispatch to a fixed keyword and are implemented in `shared/ops/` namespaces. Because shared code cannot require backend namespaces directly, implementations access backend component state via `resolve` on a `^:dynamic` var defined in `grpc/server.clj` and bound per-request by the gRPC handler. The frontend calls these functions as `SRV/*` through the existing transport with no new protobuf required. See [POLYMORPHIC_API_GUIDE](../guides/POLYMORPHIC_API_GUIDE.md) for the full pattern description.
