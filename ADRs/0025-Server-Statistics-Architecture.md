...
- [Complete Statistics Field Specification](#complete-statistics-field-specification)
  ...
  - [Raw vs. Derived Statistics Architecture](#raw-vs-derived-statistics-architecture)
    - [Design Principle: Store Raw Data, Compute Analytics On-Demand](#design-principle-store-raw-data-compute-analytics-on-demand)
    - [Benefits of This Architecture:](#benefits-of-this-architecture)
  - [Statistics Collection Points](#statistics-collection-points)
    - [API Request Processing](#api-request-processing)
    - [Event Delivery Processing](#event-delivery-processing)
...
- [Health Monitoring Integration](#health-monitoring-integration)
  - [Dual Health System Architecture](#dual-health-system-architecture)
    - [1. gRPC Health Service (Standard Protocol)](#1-grpc-health-service-standard-protocol)
    - [2. HTTP Health Endpoints (External Monitoring)](#2-http-health-endpoints-external-monitoring)
    - [System Relationship](#system-relationship)
  - [Foundational Computed Statistics Implementation](#foundational-computed-statistics-implementation)
    - [Current Basic Health Endpoint (/health)](#current-basic-health-endpoint-health)
    - [Response Structure](#response-structure)
    - [Time Unit Conversion Architecture](#time-unit-conversion-architecture)
    - [Computed Statistics Architecture](#computed-statistics-architecture)
    - [Field Naming Convention](#field-naming-convention)
    - [Integration with gRPC Server Component](#integration-with-grpc-server-component)
    - [TLS Configuration](#tls-configuration)
    - [Foundation for Comprehensive Statistics Extension](#foundation-for-comprehensive-statistics-extension)
  ...
  - [External Monitoring Tool Integration](#external-monitoring-tool-integration)
    ...
    - [Comparative Completeness Assessment](#comparative-completeness-assessment)
...
TLS configuration is **not** reported through `/health` — it is launch-time configuration, not runtime state. See [ADR-0020 §Health Endpoint Integration](0020-TLS-Infrastructure-and-Deployment-Architecture.md#health-endpoint-integration) for the rationale.
...
TLS state is deliberately **not** included in `/health` or `/health/server` responses. It is launch-time configuration (set once at startup, never changes during runtime), not state requiring polling. Operators verify TLS configuration via startup logs (`🔒 TLS encryption enabled` / `🔓 Running in plain text mode`), and inspect certificate details with standard tools (`openssl x509 -in <cert> -text -noout`). Certificate expiry monitoring is the province of cert lifecycle automation (`cert-manager`, `certbot`, ACM), not application-level health endpoints. See [ADR-0020 §Health Endpoint Integration](0020-TLS-Infrastructure-and-Deployment-Architecture.md#health-endpoint-integration) for the rationale.
