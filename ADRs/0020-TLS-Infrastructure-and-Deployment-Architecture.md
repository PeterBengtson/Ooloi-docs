...
4. [Deployment Scenarios and Certificate Management](#deployment-scenarios-and-certificate-management)
   ...
   - [Local Development (No TLS) - Default](#local-development-no-tls---default)
   - [Local Development with TLS (Self-Signed Certificates)](#local-development-with-tls-self-signed-certificates)
   - [Multi-Client Development (Distributed Testing)](#multi-client-development-distributed-testing)
...
8. [Known Temporary Limitation: Secure Peer-to-Peer Trust](#known-temporary-limitation-secure-peer-to-peer-trust)
...
11. [Alternatives Considered](#alternatives-considered)
    - [1. TLS Always Enabled](#1-tls-always-enabled)
    - [2. Third-Party Certificate Management Only](#2-third-party-certificate-management-only)
    - [3. Always Override Authority to Localhost](#3-always-override-authority-to-localhost)
...
- Integration tests verifying behavior through component lifecycle including real TLS server
...
TLS state is **launch configuration, not runtime state**, and is therefore not exposed on the HTTP health endpoints. Whether TLS is enabled or not is determined once at server startup from environment variables and configuration; it does not change while the server is running. The information available from a `/health` reading of TLS state (enabled flag, certificate path, subject, issuer, serial, validity dates) is verifiable from sources that are simpler and more authoritative than an HTTP polling endpoint:

- **Startup logs** print `🔒 TLS encryption enabled` or `🔓 Running in plain text mode` once during initialisation. Operators reading the boot log learn TLS state without an HTTP round-trip.
- **Certificate details** (subject, issuer, validity, expiry) are obtained directly from the cert file via standard tools: `openssl x509 -in /path/to/cert.crt -text -noout`. This is the canonical tool for cert inspection and is what compliance auditors use.
- **Certificate expiry monitoring** is the province of cert lifecycle automation — `cert-manager` (Kubernetes), `certbot` (Let's Encrypt), ACM (AWS), and similar. Application-level expiry alerting via `/health` duplicates what these tools already do, less reliably (the application has to be running to be queried; cert-manager works from cert metadata directly).
- **TLS handshake success** is the operative liveness signal: if a gRPC client cannot complete a TLS handshake, the connection fails long before any HTTP `/health` JSON could be parsed. Load balancers and clients observe this directly.

Removing TLS state from `/health` also resolves a structural problem introduced when the HTTP server became transport-independent (see [ADR-0036 §Hybrid Transport Architecture](0036-Collaborative-Sessions-and-Hybrid-Transport.md)). The HTTP server depends only on shared backend state (connection registry, server statistics, gRPC health manager); it deliberately does not hold a reference to any individual gRPC server. TLS state, however, is **per-server** configuration (intentionally distinct per ADR-0036), and in a configuration with multiple gRPC servers (always-on in-process + on-demand network) there is no single "system TLS state" that could be meaningfully reported. The HTTP endpoint reports state that is genuinely system-wide (status, uptime, connections, statistics, memory, GC, threads); per-server configuration is reported elsewhere.

What `/health` does include is documented in [ADR-0025 §Health Endpoint Architecture](0025-Server-Statistics-Architecture.md).
