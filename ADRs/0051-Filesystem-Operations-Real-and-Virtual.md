...
2. **A small operation set:** `list-filesystem-roots`, `list-filesystem-directory`, `open-piece`, `save-piece`, `clone-piece`, `delete-piece`, `piece-location`, and — defined but deferred (see Future Considerations) — `move-piece` (§3).
...
**Links resolve to their target.** A listing entry that *points to* a directory — a POSIX symbolic link, or a macOS Finder alias — is a directory to the user and is classified `:directory`, navigable like any other. The backend follows the whole chain to the final on-disk target (symlink→symlink, alias→alias, and mixed symlink↔alias hops) and addresses the entry's token at that target, so navigating the link descends into the real directory; the same applies when the target is a file. Symlinks resolve through the JVM (`getCanonicalFile`); macOS aliases are opaque bookmark files that `.isDirectory` cannot see through, so a small confined native step (CoreFoundation via JNA, macOS-only) resolves them. This is *filesystem-browsing correctness* — following a link to what it denotes — and is distinct from the deferred **favourites/bookmarks** (Future Considerations): those are persistent, user-curated *application* references to a location, not entries the filesystem itself presents.
...
**`piece-location` answers "where is this piece stored?" — as tokens, never a path.** For a piece with a recorded location it returns a `dir-token` and a `file-token` minted for the calling client (§6, reusing an existing token where one already maps to that location); for a piece that has never been saved — a New'd piece — it returns `nil`. The frontend learns the location only as opaque tokens (§1); *has a location* is simply `(some? …)`. This is the query the Save action consults to choose its path: no recorded location → prompt through the picker in save mode; a location recorded → write directly with a plain `save-piece`, no picker. The recorded location lives on the piece's provenance in the piece manager (the authoritative record), so the query is transport-blind and correct whether the piece is local or on a collaboration host — the frontend never reaches into backend state to read a path, which would break both the boundary (§1) and frontend/backend separation (ADR-0001).

**File extensions: `.ooloi` and `.ool`.** Ooloi recognises two piece-file suffixes, treated identically. On the read side, `list-filesystem-directory` types a file ending in either as `:piece` (everything else is `:other`); `open-piece` itself does not gate on the suffix — it validates the bytes (§7), and the picker may hand it any file the user selected. On the write side, `save-piece` **enforces** a known suffix: it appends `.ooloi` to the destination leaf unless the leaf already ends in `.ool` or `.ooloi`, so the written file — and the location recorded on the piece's provenance, hence the projected `:filename` and the window title ([ADR-0052](0052-Change-Detection-and-Event-Generation.md) §3a) — always carries one. The frontend does not enforce this; the write boundary does. `.ooloi` is the default the enforcement adds; `.ool` is the accepted shorter alternative, never generated.
...
Navigation tokens are per-client backend state. They are minted as the backend lists a directory and held on that client's entry in the shared connection registry (ADR-0036 / ADR-0024) under a `:nav-tokens` key, beside the existing `:piece-subscriptions`. The calling client is identified by the client-id the authentication interceptor places in the gRPC context (ADR-0021); an operation reads that id, consults that client's tokens, and resolves only tokens it minted for that client. One client therefore cannot resolve another's tokens, and a guessed or forged token simply misses. A token is minted **once per (client, location) and reused**: re-listing a directory — or re-calling `list-filesystem-roots` — returns the *same* token for each entry rather than minting afresh, which is what keeps the per-session count bounded (the diagram's *reused within the session*). A miss is never silent: under the uniform error contract (no silent nils — [POLYMORPHIC_API_GUIDE §Error Surfacing](../guides/POLYMORPHIC_API_GUIDE.md)), an unresolvable token throws and surfaces at `SRV/*` as **`No such filesystem token`**, and a token that resolves to the wrong type throws likewise — **`Filesystem token is not a directory`** from `list-filesystem-directory`, while `open-piece` makes the equivalent token-miss and not-a-file checks but reports them — and every other known open failure — as **typed data** rather than throwing (§7).
...
This thrown-exception path is the **backstop for the genuinely unexpected** — the failure nobody foresaw. On the combined desktop application such a thrown failure is caught off the JavaFX thread by the general error net and surfaced as a generic internal-error notification (the standalone backend has no UI and simply logs it — ADR-0036 §Notification Model). A *known* failure of a specific operation must not travel this path; it is caught at source and returned as typed data, below.

#### Known open failures: typed data, not exceptions

`open-piece` is the operation that makes the distinction explicit, because a user points it at arbitrary bytes: a file that is not a piece, a truncated or corrupt one, a file that vanished between listing and open, one the caller may not read, one too large for the heap, or one whose embedded UUID collides with a piece already open. None of these is a defect in Ooloi — each is an expected outcome of opening a file, and each deserves its own message. So `open-piece` catches them at source and returns them as **typed data**: a successful API result of the form `{:open-failure <type>}`, never a thrown exception. The frontend discriminates by shape — a bare piece-id string is success, a map is a failure — and translates the `:open-failure` type through `tr` into a specific notification. Only the unforeseen escapes to the thrown-exception backstop above.

The taxonomy is **decoupled from the picker**: `open-piece` validates the bytes itself and assumes nothing about how the file-token was chosen. The picker may hand it any file the user selected — not only a `.ooloi` — so a legitimately-selected non-piece file is a normal path here, not an edge case.

| `:open-failure` type | Condition |
|---|---|
| `:invalid-file-selection` | the token does not resolve, or resolves to a directory rather than a file |
| `:file-missing` | the file existed at listing time but is gone when opened |
| `:file-access-denied` | the file cannot be read — a permission or I/O denial |
| `:invalid-piece-file` | the bytes are not a readable piece — not Ooloi's serialised format, or a damaged/truncated one (the deserialiser cannot tell these apart; ADR-0007) |
| `:not-an-ooloi-piece` | the bytes deserialise cleanly to a value that is not a Piece |
| `:insufficient-memory` | deserialising exhausts the available heap — a very large piece, or a decompression bomb; the heap limit is JVM-configurable |
| `:piece-uuid-collision` | the piece's embedded UUID is already managed under a *different* file provenance — a variant collision (ADR-0012), detected on open |
| `:piece-uuid-mismatch` | (`open-by-uuid` only) the file at the catalogue's recorded location holds a *different* piece — its embedded UUID is not the one requested (the file changed since it was catalogued). Checked before registering, so the mismatched piece is never registered |
| `:piece-not-in-catalogue` | (`open-by-uuid` only) the requested UUID has no entry in the persistent catalogue — its location is unknown, so it cannot be opened |
| `:piece-not-found` | (`open-by-uuid` only) the catalogue has an entry for the UUID, but the file at that recorded location is gone (moved or deleted outside Ooloi) — a stale locator; the piece cannot be found at its known location |
| `:open-failed` | any other, unforeseen failure of the open operation — the catch-all, so no known-path failure reaches the backstop |
