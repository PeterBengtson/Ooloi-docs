...
- **Piece access registry**: The host tracks which pieces each collaborator (by email) has been granted access to. This registry is the predicate the filesystem-operations contract ([ADR-0051](0051-Filesystem-Operations-Real-and-Virtual.md)) filters its directory listings through: a connected guest sees only the pieces their email is authorised to reach — never the host's full library, never its filesystem. Essential for server deployments where the piece library may contain hundreds of scores belonging to different users.
...
Enablement is kept current by the menu refresh deriving its state from a **single live source**, not from state threaded in by each caller. `refresh-menu-text!` (the UI Manager) re-evaluates every menu item's `:enabled?` predicate against the live combined-app system map, which it reads from a system-state provider installed once by `start-app!` (`set-system-state-fn!`). Because the refresh always reads the one source, it is **idempotent** — any trigger produces the same correct enablement, so the menu cannot be left stale by whichever refresh happens to run last.

Collaboration state changes reach that refresh through the `:collaboration` event-bus category (ADR-0031). Every collaboration-state mutator announces `:collaboration-state-changed`:
...
A single subscriber, wired in `start-app!`, refreshes the menu on each such event. Routing every collaboration transition through this one seam is what guarantees the involuntary reversion — which has no menu action handler of its own, only the `switch-to!` revert path — refreshes like every other transition; earlier per-transition refresh calls scattered across the action handlers had left that path stale.
...
**Collaboration palette window.** While at least one network guest is connected, Ooloi shows a small floating window — a horizontal capsule (pill), always-on-top, transparent-styled, decoration-less — containing the `⇄` glyph in `-color-success-fg` on a neutral `-color-bg-default` surface (the theme's standard window background, so the pill adapts to dark/light automatically) with `Styles/ELEVATED_3` elevation. The presence signal is the green glyph, not the surface: the glyph breathes via a continuously-varying opacity cycle (per-cycle randomised duration and amplitude — no metronome cadence), fading through shades of green from full to near-transparent. Keeping the colour in the glyph rather than the surface lets the breath stay a theme-reactive token, and gestures at the project's organic aesthetic without consuming additional UI real estate. This is the first concrete instance of the **ambient indicator** category — chrome-less, shaped, always-on-top — defined in ADR-0042 §"Established Usage Patterns: Floating windows → Ambient indicators". It is a distinct category from tool palettes and from ordinary non-modal decorated windows (the `show-window!` baseline — e.g. a MIDI-playback transport); they do not share its spec-key profile.
...
The palette is built via the standard `show-window!` machinery — never bypassing the UI Manager — using the floating-window spec-key combination documented in ADR-0042 §"Established Usage Patterns: Floating windows." Three small extensions to `build-window!` / `show-window!` support this and any future ambient indicator: `:window/always-on-top?`, `:window/preserve-previous-focus-on-open?`, and auto-transparent Scene fill inferred from `:window/style :transparent`. The focus-preservation key is the JavaFX-idiomatic workaround for the absence of `Window.setFocusable(false)`: a window opening must not steal focus from the user's current input context. It works by letting the pill take focus and handing it straight back, because JavaFX cannot show a Stage without activating it — so the pill's focus-gain is real, and declaring the key is also what keeps it out of the UI Manager's focus-order record. That exclusion is not cosmetic: without it the pill would sit in the record immediately behind whichever window regained focus, and closing that window would raise and focus the pill instead of a score.

Clicking the palette body dispatches `:collaboration/show-collaborators` — the future view enumerated in the Collaboration API below. A hover-reveal close badge — a small circular button at the pill's NE corner bearing the `mdal-close` icon, hidden until the cursor is over the pill — dispatches `:collaboration/dismiss-floating-palette`. The dismissal contract is controlled by the user via a new app setting `:collaboration/floating-palette-dismiss-behaviour` with three modes: `:this-run` (closed until next app start), `:until-next-connection` (closed until the next 0→1 transition — the default), and `:always` (closed across app restarts; user re-enables via Settings).

Position memory inherits the standard `wire-geometry-listeners!` mechanism — geometry is persisted continuously (debounced) during the window's lifetime, so the palette reopens where it was, even across unclean app exits.

The collaboration palette is an **ambient indicator** in the sense of ADR-0042 §"Established Usage Patterns: Floating windows" — chrome-less, shaped, always-on-top — and inherits that category's first-open-position and drag-to-reposition behaviour. Only the *policy* below is collaboration-specific.

**First-open position policy.** On the first-ever open in a fresh install (or after a user-settings reset) there is no persisted geometry. An ambient indicator must supply explicit `:x`/`:y` (a chrome-less window otherwise auto-centres — wrong for an ambient surface). The collaboration palette computes those coordinates as the right-hand screen corner *vertically opposite* the notification overlay, inset by the overlay margin: the notification overlay is placed by the `:ui/notification-position` app setting (`:bottom-right` — the default — or `:top-right`), and the palette mirrors it on the same right edge — notifications at the bottom-right ⇒ palette opens top-right, notifications at the top-right ⇒ palette opens bottom-right. The two ambient surfaces thus occupy opposite right-hand corners and never collide. The persisted-geometry merge in `show-window!` overrides these coordinates whenever saved geometry exists, so the mirror rule governs only the first open and the user's chosen position governs every subsequent one. Drag-to-reposition (the user moving the chrome-less pill) is the generic ambient-indicator gesture documented in ADR-0042; the body click within the drag threshold dispatches `:collaboration/show-collaborators`.

All user-facing strings (the tooltip on the close badge, the body-click hint, the setting's label/description/choices) are tr-driven per ADR-0039.

**Code organisation.** The collaboration palette lives in `frontend/ui/app/collaboration_palette.clj` — `show-collaboration-palette!`, `close-collaboration-palette!`, and the cljfx content spec. The StackPane wrapper, drop-shadow elevation, and hover-reveal close badge are provided by the generic helper `palette-frame-spec` in `frontend/ui/core/palette.clj`; this namespace supplies only the collaboration-specific pieces (centre glyph, background colour, dismiss event, button id). Lifecycle wiring lives in `shared/system.clj` as `wire-collaboration-palette-lifecycle!` (the connection-registry atom watch). A future tool palette would be a sibling namespace `frontend/ui/app/<name>_palette.clj` reusing both the `palette-frame-spec` helper and the same generic floating-window spec keys; the *name* "floating palette" is the architectural category, not the file name.
