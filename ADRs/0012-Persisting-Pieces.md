...
- [Detailed Design](#detailed-design)
  ...
  - [2. Integer ID Reference Management](#2-integer-id-reference-management)
  - [3. Serialization and Deserialization](#3-serialization-and-deserialization)
  - [4. Piece Identity: UUID as Primary Key](#4-piece-identity-uuid-as-primary-key)
- [Rationale](#rationale)
...
**Regenerated only on server transfer — explicitly.** When a piece is registered on a *different* server (uploaded, or copied into a new pool), its UUID is regenerated, the old UUID is discarded, and the new UUID is written back to the file before registration. This is an explicit, auditable act, not automatic: two copies of one original file on two servers are correctly two different pieces with different UUIDs. Cloning — duplicating a piece — likewise produces a new UUID; the copy is a different piece that merely shares content at the moment of cloning, and their histories diverge from that point.

**Cloning regenerates identifiers only, not content.** Duplicating a piece (Save As, [ADR-0051](0051-Filesystem-Operations-Real-and-Virtual.md)) regenerates every **structural** identifier — the piece's string UUID *and* every Musician/Instrument/Staff/Layout `:id` (the keyword ids of *Three distinct identifiers* below) — so the copy shares no identity with the original, and the two can be open side by side without their selections or drag-and-drop colliding. It regenerates **only** those identifiers. Everything else is shared with the original by reference at the moment of cloning: all musical content, the change-sets, names, transpositions, ranges — every non-identifier value. The model is immutable, so this sharing is safe; a later edit to either copy copies-on-write, and the two histories diverge from there. There is one exception to *shared by reference*: a Layout's `:musician-uuids` holds the ids of musicians in the same piece ([ADR-0053](0053-Piece-Window-and-Piece-Preferences.md)), and those ids are among the ones regenerated, so cloning **remaps** each layout's `:musician-uuids` to the new musician ids rather than sharing the original's — otherwise the copy's layouts would point at the original's musicians.
...
- **Collision detection on open** — the Piece Manager detects when a piece being opened carries a UUID already held by a different, currently-managed piece, and warns the user rather than producing undefined behaviour. "Different" is decided by **file provenance** — the resolved file's path and modification time, recorded in the registry entry beside the managed ref. Reopening the *same* file (same UUID, same provenance) is idempotent: the UUID is returned and the managed ref preserved, with no re-registration. A *different* provenance for an already-managed UUID — a different path, or the same path with a changed modification time — is the variant collision detected here; it is reported as a thrown error carrying the existing and incoming provenance, and never silently overwrites the incumbent.
...
**The durable UUID → location index.** Resolving a UUID to *where its piece is stored* — needed to re-open a piece no longer held in memory, as single-user session restore does — is a durable **catalogue** the Piece Manager owns beside its in-memory registry: embedded UUID → a tagged, storage-opaque locator (`{:kind :filesystem :path}` today), recorded for every piece whether open or not. It is a location *index*, not an identity registry — identity stays embedded in the piece (above); the catalogue only records where each UUID's piece currently lives, and a stale entry degrades gracefully rather than corrupting identity. Its operation surface — `open-by-uuid` and the typed-failure taxonomy — is specified in [ADR-0051](0051-Filesystem-Operations-Real-and-Virtual.md); its store mechanics (written through the provenance recorders, persisted on every mutation, best-effort) are in the [Piece Manager Guide](../guides/PIECE_MANAGER_GUIDE.md).
...
- **Name → `:title`** — the piece's human work name, and piece *data*: round-tripped by `get-title`/`set-title`, present in the `get-piece-structure` projection ([ADR-0052](0052-Change-Detection-and-Event-Generation.md)), user-editable, blank (`""`) by default; the window title derives from it, falling back to the filename (below) and then to "Untitled" via `tr` when both are blank.
- **Location → filename** — external **provenance**, and *never* piece data: the resolved file's path and modification time, held only in the Piece Manager registry entry beside the managed ref (for the collision detection above). It is never embedded in the piece and never *written into* piece data — but `get-piece-structure` does surface it as a **virtual `:filename` field**, conj'd at projection time from the provenance (the leaf, never the path — [ADR-0051](0051-Filesystem-Operations-Real-and-Virtual.md)), so the window title can fall back to it. (A blank `:title` therefore takes the filename, its `.ooloi`/`.ool` extension stripped, as the window-title default — a name derived *from* the filename — while the filename itself is never written *into* piece data.)
...
3. **Structural entity UUIDs** ([ADR-0010](0010-Pure-Trees.md) / [ADR-0011](0011-Shared-Structure.md)) — keyword `:id` slots on Musicians, Instruments, Staves, and Layouts (each embedding a UUID, e.g. `:musician-<uuid>`), generated at creation, whose purpose is GUI selection and drag-and-drop survival across the invalidate/refetch cycle. UI identity — neither piece identity nor cross-element reference.
...
- [ADR-0053: The Piece Window and Piece Preferences](0053-Piece-Window-and-Piece-Preferences.md) - Introduces `:musician-uuids`, the layout-to-musician references cloning must remap
