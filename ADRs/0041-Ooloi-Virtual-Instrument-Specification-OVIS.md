# ADR-0041: Ooloi Virtual Instrument Specification (OVIS)
...
- [Bundled OVIS Files](#bundled-ovis-files)
...
OVIS addresses the playback control requirement. It defines how playback plugins communicate with virtual instruments using modern host control mechanisms rather than OS-level MIDI routing.
...
OVIS generalises the Synth Matrix concept from MIDI synthesisers to virtual instrument libraries. The underlying principle remains: declarative text files describing device capabilities, user-manageable, and distributable. What changes is the control surface (VST3/AU host mechanisms rather than MIDI) and the scope (articulation-level detail rather than patch-level).
...
VST3 and AU provide different implementations of these capabilities. VST3 offers IKeyswitchController and INoteExpressionController; AU has different mechanisms. OVIS abstracts over format differences, describing capabilities rather than interface specifics.

OVIS leverages these modern capabilities while preserving both the musical intelligence and the user-manageable device description architecture that made Igor's approach effective. Unlike Igor, OVIS does not output MIDI; playback goes directly to virtual instruments via host control. Users who need MIDI output to external hardware would require a dedicated plugin—which could reimplement Igor's sophisticated allocation strategies or use simpler fixed-channel mapping.
...
OVIS addresses calibration through mandatory loudness normalisation, onset alignment parameters, and fallback generators for missing articulations.

## Decision

We adopt a **declarative YAML-based specification** for virtual instrument control that:
...
**Declarative over procedural**: OVIS files describe what exists and how to access it, not how to implement playback logic.

**Canonical abstraction**: Vendor-specific names map to a standard taxonomy. The playback engine works with `legato_portamento`; the OVIS file translates this to Synchron's `["Legato","Portamento"]` dimension path.
...
**MIDI output would require a plugin**: If a user wanted MIDI output to external hardware synthesisers, they would need a dedicated plugin. Such a plugin could be simple (fixed channel per instrument) or sophisticated (Igor-style dynamic channel allocation with pitch bend for microtonality). OVIS does not address MIDI output; it addresses control of virtual instruments within the plugin host.
...
**Format-neutral capabilities**: The specification describes capabilities (parameter automation, articulation selection, per-note expression) rather than format-specific interfaces. VST3 and AU provide different implementations of these capabilities; OVIS abstracts over them.
...
The taxonomy provides vendor-agnostic names for articulations, mapped to notation semantics. Playback plugins work with canonical names; OVIS files provide the vendor-specific translation.
...
Articulations have different attack characteristics. OVIS compensates through:
...
When `native: false`, the OVIS file specifies a fallback generator:
...
Individual OVIS files describe instruments. Library manifests describe what instruments exist within a library, enabling the playback engine to determine availability without scanning individual files.
...
**Human-readable**: Musicians and developers can inspect and modify OVIS files without specialised tooling.
...
**Proven precedent**: Igor Engraver's Synth Matrices demonstrated that user-manageable text files describing device capabilities enable community support for diverse hardware without centralised development. OVIS continues this architecture with a modern format.
...
**Notation semantics linkage**: Canonical names map to notation concepts. The playback engine translates a tremolo marking to `tremolo`; the OVIS file translates `tremolo` to vendor-specific selection.

**Extensibility**: New techniques add to the taxonomy without breaking existing OVIS files.
...
**MIDI output remains possible via plugins**: Nothing prevents creation of a MIDI output plugin for users who need external hardware control. Such a plugin could implement simple fixed-channel allocation or sophisticated Igor-style dynamic routing. This is outside OVIS scope.

## Challenges Addressed

### Vendor Fragmentation

OVIS files isolate vendor-specific knowledge. The playback engine works with canonical techniques; OVIS translates to vendor implementations. Adding support for a new library requires only an OVIS file, not engine modifications.

### Version Drift

Libraries update, changing parameter names and keyswitch assignments. OVIS files can specify minimum versions and be updated independently of the playback engine. Version tracking fields (`library_version`, `content_revision`) enable managing this evolution.
...
OVIS operates entirely within the plugin layer established by ADR-0027. It specifies how playback plugins control virtual instruments, not how the core handles audio (which it does not).

### ADR-0003: Plugin Architecture

OVIS files are resources consumed by playback plugins. The plugin architecture's resource management, versioning, and distribution mechanisms apply. Community-contributed OVIS files can be distributed through the plugin ecosystem.
...
OVIS controls *what* sounds play. A future humanisation specification will control *how* they play—tempo variation, dynamic shaping, rubato, ensemble timing. Humanisation plugins will consume OVIS-controlled instruments, adding interpretation layers. OVIS must be stable before humanisation can be specified.
...
**Rejection**: Does not scale. Every new library requires plugin code changes. Community contribution requires programming expertise. No separation between "what exists" (OVIS files) and "how to use it" (engine logic).
...
- **Selection reliability**: Articulation selection should succeed when the target articulation exists and the OVIS file correctly describes the selection mechanism.
...
- **Library coverage**: OVIS files available for major orchestral libraries (BBCSO, Synchron, Berlin, Hollywood, SWAM)
- **Community contribution**: Third-party OVIS files accepted and maintained
...
**Calibration harness**: Automated tool that loads instruments, plays reference material, measures loudness and onset timing, and generates calibration values for OVIS files.

**Community registry**: Central repository for OVIS files with version tracking, validation, and search by library/instrument/technique.

**Extended taxonomy**: Contemporary extended techniques, non-Western instruments, electronic/synthesis articulations.

**Humanisation integration points**: Defined interfaces for humanisation plugins to modify OVIS-controlled playback.
...
OVIS embodies Ooloi's architectural philosophy: solve the problem correctly at the right layer, use modern capabilities rather than legacy workarounds, and enable community contribution through well-defined interfaces.

## Appendix A: Vendor Control Surface Research

This appendix documents the control mechanisms available for major virtual instrument platforms, providing the empirical foundation for OVIS design decisions. The research focuses primarily on VST3 implementations; AU behaviour may differ in specifics while providing equivalent capabilities.

### VST3-Specific Interfaces

These interfaces are VST3-specific. AU provides different mechanisms for similar capabilities; OVIS abstracts over format differences at the specification level.

**Parameter automation**: All VST3 plugins expose automatable parameters. OVIS leverages this for articulation selection and expression control.
...
**Ideal OVIS fit**: Direct parameter control without keyswitch complexity.
...
**OVIS approach**: Use event-based selection within the plugin host. No MIDI output involved.
...
- [BBC Symphony Orchestra Discover](https://www.spitfireaudio.com/bbc-symphony-orchestra-discover) — Bundled library (primary)
- [Virtual Playing Orchestra](https://virtualplaying.com/virtual-playing-orchestra/) — Bundled library (fallback coverage)
