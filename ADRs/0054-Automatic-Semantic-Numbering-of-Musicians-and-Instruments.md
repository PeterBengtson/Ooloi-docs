...
- **A warning notification** when the kind is left or made discontinuous — an addition that fills one hole but leaves another (1, 4 becomes 1, 2, 4), or a deletion that opens a hole (1, 2, 3 becomes 1, 3).
- **Silence** when nothing is notable — a plain append onto a consecutive kind, a deletion off the top that leaves the rest consecutive, and the two-becomes-one clearing (which leaves a single unnumbered instrument, not a hole).

Green comes only from a gap-closing addition; a deletion can never produce it, because a deletion never renumbers and so can never close a hole. "Closed" means *fully* consecutive: an addition that closes one hole while another remains is a warning, not a green, because the kind is still discontinuous.
...
The numeral wraps the **whole** name, transposition qualifier included — never the middle of it. With the defaults (Arabic, precede, period on) a third trumpet in B♭ is `3. Trompete in B♭`; set to Roman and follow it is `Trompete in B♭ III`; Roman and precede gives `III Trompete`. The full stop is the Arabic-precede convention alone; every other combination separates numeral and name with a single space.
