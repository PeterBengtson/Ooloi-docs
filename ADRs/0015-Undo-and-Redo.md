...
- [Rationale](#rationale)
  - [STM Provides Natural Collaborative Undo/Redo](#stm-provides-natural-collaborative-undoredo)
  - [Clear Separation Prevents Architectural Complexity](#clear-separation-prevents-architectural-complexity)
  - [Backend Settings Analysis Reveals Minimal Needs](#backend-settings-analysis-reveals-minimal-needs)
...
- [Alternatives Considered](#alternatives-considered)
  - [Alternative 1: Unified Global Undo/Redo](#alternative-1-unified-global-undoredo)
  - [Alternative 2: Piece-Only Settings (Everything in Piece Data)](#alternative-2-piece-only-settings-everything-in-piece-data)
  - [Alternative 3: Event Sourcing on Top of STM](#alternative-3-event-sourcing-on-top-of-stm)
  - [Alternative 4: Backend Application Settings Component](#alternative-4-backend-application-settings-component)
...
2. **Backend unreachable**: Tier 2 entries — the frontend's own UI history — remain undoable, because they never needed the backend. When the backend is unreachable and its cached timestamp is the highest, the undo item can show the backend entry as disabled (greyed) while local entries stay available. This is a crash-recovery state, not an offline working mode: there is no offline editing of a piece, because a client with no backend has no piece ([ADR-0040](0040-Single-Authority-State-Model.md), where "offline" means "the backend crashed" and operation queueing for offline support is explicitly rejected).
