...
- **[0052-Change-Detection-and-Event-Generation](0052-Change-Detection-and-Event-Generation.md)**: How a backend mutation becomes an event — detection at the single VPD write funnel, a declaratively-conferred trait emitting `:piece-structure-changed` for structural changes, agent-deferred emission coalesced to exactly one event per outermost transaction, the per-piece dirty flag held by the Piece Manager outside the piece value, and the coarse-structure versus fine-formatting invalidation regimes that drive the frontend's invalidate→refetch
...
- **[0038-Backend-Authoritative-Rendering-and-Terminal-Frontend-Execution](0038-Backend-Authoritative-Rendering-and-Terminal-Frontend-Execution.md)**: Backend-authoritative rendering with terminal frontend execution and GPU-accelerated vector rendering
- **[0039-Localisation-Architecture](0039-Localisation-Architecture.md)**: Frontend-only localisation with PO files, single translation API, and build-time verification — see the [Translator's Guide](../guides/TRANSLATORS_GUIDE.md) for the translator-facing companion
...
- **[0054-Automatic-Semantic-Naming-and-Numbering-of-Musicians-and-Instruments](0054-Automatic-Semantic-Naming-and-Numbering-of-Musicians-and-Instruments.md)**: Naming and numbering as a consequence of the drop rather than a task it leaves behind — the piece-wide instrument axis counted by exact name (transposition included), the musician taking its name, short-name and number from its main instrument, the freeze that writes only to `nil` beside the comparison that overwrites only the automation's own output, holes reported by a non-blocking signal instead of silently closed, three numeral settings composing the shown label, and the doubling annotation
