...
- **Phase 4 - Connection Broadcast**: Server sends `server-client-connected` event to ALL connected clients (including the new client) — but only on a **network** server. The in-process backend suppresses this collaboration broadcast: a single-client local backend ([ADR-0040](0040-Single-Authority-State-Model.md)) has no collaboration peers to inform.

**Disconnection Events**:
- **Phase 5 - Graceful Disconnect (RPC)**: Client invokes the unary `Disconnect` RPC (typically from `switch-to!` during transport switching, or at application shutdown). The server's `handle-disconnect` reads the client-id from the gRPC context (populated by the auth interceptor from `CLIENT_ID_HEADER`), atomically dissocs the registry entry via `swap-vals!`, releases the client's pieces — unsubscribing it from each and closing any piece thereby left with no subscribers (the close-on-last-release of [ADR-0022 §Piece Lifetime](0022-Lazy-Frontend-Backend-Architecture.md)) — shuts down the per-client drainer executor, and broadcasts the disconnect event (PHASE 6). The handler does **not** call `.onCompleted` on the streaming `RegisterClient` call from this code path: the same client typically owns both the Disconnect unary call and the streaming call, and a server-initiated `.onCompleted` would fire client-side teardown that closes the API channels — including the one carrying the Disconnect's still-in-flight response — causing a self-closing race. The streaming call is cleaned up instead via PHASE 7's cancel-handler backstop after the client tears down channels.
- **Phase 6 - Disconnect Broadcast**: Server sends `server-client-disconnected` event to ALL remaining connected clients. Emitted only on a **network** server — the in-process backend suppresses collaboration broadcasts (see Phase 4) — and gated on whether the dissoc actually changed the registry (`(not= old new)` from `swap-vals!`), so an idempotent no-op Disconnect (client-id already absent) does not emit a spurious broadcast.
- **Phase 7 - Cancel-Handler Backstop**: When the client tears down its channels (after Disconnect returns, or involuntarily on network drop / JVM exit / server-initiated kick), the gRPC layer fires `setOnCancelHandler` on the server. This handler is **identity-aware**: it captures the response-observer in its closure at registration time, and at fire time atomically dissocs the registry entry only if the current entry's `:server-obs` is `identical?` to the captured one, then releases that client's pieces just as the graceful path does (close-on-last-release, [ADR-0022 §Piece Lifetime](0022-Lazy-Frontend-Backend-Architecture.md)). This protects re-registered entries (post-`switch-to!`) from being wiped by stale cancellations and ensures the involuntary-disconnect path produces the same observable disconnect event as the graceful path. The drainer executor is shut down unconditionally — its lifecycle is per-stream.
