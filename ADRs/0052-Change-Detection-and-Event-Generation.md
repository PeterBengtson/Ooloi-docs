...
That is the whole reason the exception is safe. An earlier version of this ADR dropped `:settings` everywhere on the grounds that it "travels on its own `:piece-setting-changed` channel and would otherwise read stale here" — correct at the time, because nothing refetched on a settings write. Being structural on the Piece removes the staleness rather than tolerating it. The other channel is unaffected and still serves its own purpose: `:piece-setting-changed` refreshes an open Preferences window's controls ([ADR-0053](0053-Piece-Window-and-Piece-Preferences.md) §6), and graphical consequences still travel as cache-invalidation. Three channels, three consumers, none doing another's work.
