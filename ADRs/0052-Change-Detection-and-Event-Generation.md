...
That is the whole reason the exception is safe: being structural on the Piece removes the staleness rather than tolerating it. The other channels are unaffected and still serve their own purposes: `:piece-setting-changed` refreshes an open Preferences window's controls ([ADR-0053](0053-Piece-Window-and-Piece-Preferences.md) §6), and graphical consequences travel as cache-invalidation. Three channels, three consumers, none doing another's work.
