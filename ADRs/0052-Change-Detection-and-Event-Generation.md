...
- [Decision](#decision)
  ...
  - [7. Formatting-pipeline invalidations](#7-formatting-pipeline-invalidations)
- [Sequence Diagrams](#sequence-diagrams)
  - [From a structural change to the refetched window](#from-a-structural-change-to-the-refetched-window)
  - [One event per outermost transaction](#one-event-per-outermost-transaction)
  - [From a Save to the retitled window](#from-a-save-to-the-retitled-window)
- [Rationale](#rationale)
...
There is a second reason, from [ADR-0016](0016-Settings.md)'s own uniformity principle: a setting must be indistinguishable from a direct slot, and a write to a direct slot of a structural entity announces itself — `set-title` and `set-name` emit, as the §3b table below records. A silent settings write therefore gave a caller promised an identical interface a difference in observable consequence. Making `:settings` structural on the Piece was required by that principle independently of what any window needs, which is why the exception corrects an inconsistency rather than introducing one. ADR-0016 §"Which settings reach the Piece Window" carries the argument in full, including why it does not yet bind below the Piece.

That is the whole reason the exception is safe: being structural on the Piece removes the staleness rather than tolerating it. And it removes it for *every* consumer at once — the Piece Window's numeral labels and a Piece Preferences window's own controls read the same projected entries, so neither can display a value the other has already been told has changed. A dedicated per-setting event was considered for the Preferences window and rejected: the refetch happens regardless, driven by the same write, and the projection it returns already carries every setting's value ([ADR-0053](0053-Piece-Window-and-Piece-Preferences.md) §6 states the reasoning). Two channels, then, not three: this one for what the piece *is*, and cache-invalidation for what the music *looks like*. Neither does the other's work.

The cost accepted is over-signalling: a write to *any* piece setting refetches the projection, including settings no label depends on. This is deliberate and cheap — piece settings change rarely, and the projection carries no measures, voices or items. Should it ever stop being cheap, the precise alternative is to make projection-bearing a per-setting declaration rather than a per-entity one; that is more machinery for precision worth little at this frequency.
...
|---|---|---|
| every node **except Piece** | `:settings` | — |
| Piece | `:time-signatures` `:key-signatures` `:tempos` | `:id` `:title` `:musicians` `:layouts` `:settings` |
...
**Where this lives, and the seam.** The gate, the scope, and the slot test (§3b) are the change-detection system, and they live in a dedicated home (`shared/ops/change_detection.clj`) — so the write funnel's location is unchanged while the detection system has room to grow the content and formatting regimes (§6–§7). Emission crosses the shared→backend boundary by **dependency inversion**: the funnel names the operation — *a structural change occurred for this piece* — through a protocol declared in the shared tier and implemented in the backend, which broadcasts on the event stream. This is the seam `PieceResolver` already uses, not a runtime lookup of a server component. A context with no backend implementation — a pure-shared test, a frontend-local value manipulation — has no subscriber and emits nothing, which is the §2 object-form exclusion seen from the other side.

**Undo capture stands beside the gate.** `with-coalesced-structural-change` exists for the structural gate alone. Piece undo capture keys on the *same* outermost-transaction boundary — one gesture, one transaction, one undo step — but does not extend this primitive: the backend gRPC handler that owns the transaction (`execute-atomic-operations` for an `SRV/atomic` batch; the single-method handler, wrapped as a batch of one, for a lone call) reads the piece value before and after that boundary and pushes one undo entry ([ADR-0015](0015-Undo-and-Redo.md)). The gate stays structural-coalescing only; undo capture sits beside it, spanning content as well as structure, and is taken only on the client-facing `SRV/` surface — a backend-internal `api/` mutation registers no undo step *automatically*, backend code that wants one registering it deliberately through `push-undo!` ([ADR-0015](0015-Undo-and-Redo.md) §Automatic Capture and Deliberate Registration). The confinement is what keeps a formatting pass — a funnel writer, and so a dirtying one (§5) — off the undo stack. Its label is derived from the operation at the boundary, not named at the mutation site.

### 5. The dirty flag

A piece is dirty when it has changed since it was last saved. Dirtiness is **OR-accumulated**: any VPD change whose resulting value is not identical to the prior value sets the flag; a change that leaves the value untouched leaves the flag as it was, so a no-op never masks earlier unsaved changes. This test is broader than the structural one of §3b: it reads the funnel's identity comparison *above* the structural-slot narrowing, so a change that emits no `:piece-structure-changed` — an added measure, a new time signature, a changed piece setting — still marks the piece dirty. Structure names the subset the Piece Window refetches; dirtiness spans every unsaved change, structural or not, and the two sets do not coincide. The flag is cleared only by a successful save, and **derived — not set —** by undo and redo, which ask whether the state they restored differs from what was last saved.

**Why a restore derives rather than carries.** Dirtiness is a relation between the piece and a file, and a restore changes only one side of it. A flag captured with the snapshot describes that relation as it stood during the edit, and a save between capture and restore moves the other side — after which the captured value inverts in both directions: an undo *away* from the saved state reports clean, a redo *back onto* it reports dirty. Nothing recorded at capture can prevent this, because what moved was not the piece.

So the Piece Manager retains the value each piece is clean with respect to: set when the piece enters the manager — the loaded value on open, the fresh value on new — and replaced by every successful save. A restore compares against it and sets the flag accordingly, reporting the flip in the same way the mutators do so the caller emits `:piece-dirty-changed` exactly once.

Forward, nothing consults it: dirtiness is accumulated at the funnel exactly as described above, and the comparison exists only for a piece that arrives at a state by some route other than editing forward into it. The comparison is `=`, as the save's own is and for the same reasons — it short-circuits on identity, and a restore writes a value the undo entry already holds, so the ordinary case costs nothing while a hash-consing sweep between save and restore leaves an equal-but-not-identical value that must still count as clean.
...
**Staleness marks are the deliberate opposite, and the contrast is the clearest way to state either.** The formatting regime's stale marks — which record that a derived visual artefact no longer reflects the inputs it was computed from — live **inside** the piece, in a top-level slot ([ADR-0028](0028-Hierarchical-Rendering-Pipeline.md) §Staleness). Both placements follow from the same question, asked of different subjects. This flag describes the piece's relationship to a **file**: it must not survive a reload, a save clears it, and undo restores rather than sets it. A stale mark describes the piece's own derived content relative to its own semantics: it **must** survive a reload, because the layout hierarchy it describes does, and a score reopened with three measures outstanding must resume that work rather than reformat from the beginning. Persistence is the discriminator — this flag must not persist, those marks must — and it is also why marks are correctly captured by undo snapshots where this flag is correctly excluded.

**A formatting pass is a dirtying writer.** The pipeline mutates the piece when it recomputes, so by the rule above — any change whose resulting value is not identical to the prior one — it marks the piece dirty. That is the rule applying rather than an addition to it: this section says *change*, never *user change*. The single existing exemption does not extend to it. The hash-consing daemon writes without dirtying because its substitutions are `=` to what they replace, so nothing about the saved file changes ([ADR-0029](0029-Global-Hash-Consing.md)); a formatting pass writes materially new content that belongs on disk. The consequences — a saved piece becoming dirty again once formatting completes, and a piece opened with outstanding marks dirtying itself with no gesture involved — are correct, and are stated with their reasoning in ADR-0028 §"The pipeline's write dirties the piece".
...
**A deferred write clears the flag only if the piece is unchanged since its snapshot, and that comparison is `=`.** Saving a piece splits in two: the snapshot is taken the instant the user asks, and the write happens later on the save queue ([ADR-0051](0051-Filesystem-Operations-Real-and-Virtual.md) §7). That opens a window in which the piece can be **edited**, since a user carries on working while a large score is being frozen. Clearing the flag on completion regardless would mark as saved a piece whose latest edit exists nowhere but memory — so the user quits without being asked and loses it. The clear is therefore conditional on the piece still being equal to the value that was written.

**The comparison is `=` and not `identical?`, and the reason is the hash-consing daemon.** A sweep landing in that same window replaces the piece value with one that is `=` to it and merely not `identical?` ([ADR-0029](0029-Global-Hash-Consing.md)). Under `identical?` the sweep would block the clear and leave a genuinely saved piece showing as modified — the daemon becoming visible, which ADR-0029 forbids outright. `=` is exactly the equivalence the daemon preserves and an edit does not.

So this document's two comparisons are used in the two places their costs fit, and the difference is not an inconsistency. The funnel's no-op gate uses `identical?` and never `=`, because it sits on the write path where a deep compare over an arbitrary subtree is O(n). This clear is not on the write path: it happens once per save, on the save queue's own thread, after the file is already written.

**What `=` costs on a score that may be an entire Wagner opera**, since "use `=`" must not be recorded as free:

- **Unchanged — O(1) at any size.** The ref still holds the very value that was snapshotted, and `=` short-circuits on identity before comparing anything. This is the ordinary case: a save with no concurrent edit and no sweep.
- **Edited — proportional to depth, not size.** `=` returns false at the first difference, and every sibling subtree on the way down is dismissed by an identity check.
- **A sweep landed in the window — a real deep traversal**, because the substituted values are `=` but not `identical?`, so identity short-circuiting cannot help. Rare, on the queue's thread, after the file is written, and it ends in the flag correctly clearing.

The cost is accepted because of where it falls: the correctness-critical question is whether the piece was *edited*, and inequality is found quickly. Only confirming equality after a transparent rewrite is slow. Were it ever to matter, the O(1) answer is a generation counter bumped by the funnel and therefore blind to the daemon by construction — not adopted, because it is new state for a question existing values can answer.

**Two write funnels, one identity test.** A VPD mutation reaches the piece through one of two funnels, and dirty reads the same `(not (identical? before after))` at each. The general funnel carries the adders, removers, movers, sequence writes, attribute writes and the `individuate` self-transform; its set mutators are the ones above, gating on `identical?` and never `=`, because the value they carry is arbitrary — a whole `:measures` vector, a subtree — where an `=` compare would be the O(n) cost the write path cannot bear. Piece settings travel through a separate funnel, `vpd-mutate-setting`, which observes the same no-op-returns-identical invariant through its own gate: `identical?` first, then `=`, on the setting value. Here `=` is admissible because a setting is a small scalar, and it is needed because a setting arriving over the wire may be a fresh object — `=` to the stored value but not `identical?` to it (a non-cached number, a string) — so an `identical?`-only gate would rebuild the piece and mark a genuine no-op as dirty. (`=` short-circuits on identity within `Util/equiv`, so the leading `identical?` reads for clarity and mirrors the general funnel's gate rather than earning any speed.) A settings write whose value differs marks the piece dirty; a **piece** setting additionally emits `:piece-structure-changed`, `:settings` being structural on the Piece (§3a), while a setting on any lower entity does not. Note that this funnel reports the slot `:settings` and not the individual setting key, which the setter closes over — so the emission is per-write, not per-setting, and cannot currently be made selective without threading that key through.
...
**A caveat on the name.** "Structural" covers more than containment. Besides who plays what, on how many staves and in which layouts, it covers each entity's configuration — which is not containment at all. The trait, the event and the multimethod carry the name regardless; read it as *everything about a piece that is not the music*, which is the line the trait actually draws, rather than literally the piece's structure.
...
### 7. Formatting-pipeline invalidations

This is the other half of the division in §6: how a change to the **music** — and to anything else that alters how the music is drawn — becomes visible. This ADR specifies where change is *detected*; the regime that turns a musical change into visible output is specified in **[ADR-0028 §Staleness: The State That Drives Incremental Formatting](0028-Hierarchical-Rendering-Pipeline.md#staleness-the-state-that-drives-incremental-formatting)**, which is its home. Read that section for the state, its structure, and the invariants; what belongs here is only how it meets this mechanism.

- **A single change may produce more than one invalidation, at different levels of the visual hierarchy** — layout, page, system, staff, measure (the levels [ADR-0022](0022-Lazy-Frontend-Backend-Architecture.md) enumerates). *Which* levels and *how many* are the formatting engine's decision, not the detector's; the sequence is consolidated by subsumption, a containing invalidation absorbing the finer ones it covers, under the rule ADR-0028 states.
- **Detection marks; the pipeline derives.** A change records a stale mark at the level where it occurred and nothing more. The fan-out — a measure implying its system, a system implying its page — is the pipeline's own knowledge of its stage dependencies, computed at formatting time and never written into the marks. This is the §1 rule seen from the formatting side: one detection point, and no downstream layer re-deriving what changed by diffing state.
- **Undo needs no part in it.** Because the marks live in the piece, an undo snapshot carries its own staleness state, so a restore is self-consistent without marking, recording touched VPDs, or diffing before against after ([ADR-0015](0015-Undo-and-Redo.md) §"A restore announces; it does not detect").
...
- **A formatting pass dirties the piece** (§5), and formatting must be a fixpoint or autosave and the pipeline drive each other indefinitely. ADR-0028 states the invariant and the reason.

These invalidations are forward staleness signals out of formatting; they do not let the renderer renegotiate semantic or layout decisions — the rendering boundary holds.
