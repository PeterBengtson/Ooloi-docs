...
As the user assembles a piece — dragging an instrument from the Instrument Library, cloning a musician, dropping a doubling into a player, deleting an instrument or the player holding it, reordering another — Ooloi numbers the musicians and instruments for them. Most of those gestures are drag-and-drop; a deletion is a keypress, and it numbers too, since removing one of a pair leaves a survivor that must lose its number (§3). The user writes no numbers, opens no numbering dialog, and enters no mode. The numbers simply appear, correct, as a consequence of the structural gesture the user already made, and the user goes on thinking about the music.
...
**The musician takes its identity from its main.** Not its number alone: a musician's `:name` and `:short-name` are its main instrument's too — seeded when the musician is created, and re-derived when a structural gesture installs a different main. They are ordinary stored fields on the Musician, not a view onto the instrument; the automation fills them in as a convenience, and the user may edit them afterwards. This is a help, not an equivalence, and the distinction matters: a player holding four clarinets of different sizes may be called `Multi-Clarinet Guy` — a name no main instrument would ever produce — and nothing in Ooloi overwrites it. The field's *content* is entirely unconstrained: it may describe the desk (`Reed 2`), or simply be the performer — `Erik`, `Michelle` — with nothing in it relating to an instrument at all. What it may not be is empty; clearing it asks for the derivation back (§3). Ooloi seeds it from the main because that is the useful default, not because the two are bound.
...
§3 states exactly when the automation writes these fields and when it leaves them alone.
...
**The musician axis is re-derived, not frozen.** The invariant above governs the instrument axis, where a number names a part that may already hold music: deciding that Oboe III is henceforth Oboe II helps nobody, so a hole in a section stands until a user closes it. A musician's fields answer a different question — *which player is this* — and a gesture that changes the player's main changes the answer. So when a gesture installs a different instrument at the top of a musician's list — a doubling dropped above the main, a deletion that promotes a doubling, a reorder that lifts one to the top — that musician's name, short-name and number are re-derived from the new main. Drag-and-drop sets up the structure, and the naming and numbering that belong to it; that is the help it exists to give, and the user is free to adjust the result afterwards.

**The automation overwrites only what it wrote itself.** Before re-deriving, each field is compared with the *outgoing* main's: where they still agree, the value is the automation's own earlier output and is replaced; where they differ, the user has taken the field over and it is left untouched. A musician still reading `Clarinet in B♭` plainly matches its old main and follows the new one; `Multi-Clarinet Guy` matched no clarinet, so no gesture renames him. There is no lock, no flag and no record of who set what — the comparison *is* the record, just as the instrument axis's protection is simply that it writes to `nil` and nowhere else. A hand-edit therefore stands, and the automation never reaches into a name it did not create. One consequence is accepted rather than solved: because the comparison cannot tell *which* edit caused a divergence, renaming the main instrument by hand also reads as the user having taken the musician's name over, and that musician then keeps its name through every later gesture. It fails in the safe direction — the automation declines to write rather than overwriting what it should not — and the remedy is a single edit to the musician's own name.

**Emptying the field asks for the derivation back.** The comparison above governs what a *gesture* may overwrite; it says nothing about what the *user* may ask for. Committing an empty name is that request: a `:name` may never be blank, so the field reverts to the main instrument's name and the derived value is written, not merely shown. A player with no main has nothing to revert to, and there a blank is refused rather than derived. The full rule, across every entity that carries a name, is in [ADR-0052](0052-Change-Detection-and-Event-Generation.md). One consequence belongs here: the restored name again equals its main's, so the header's parenthesis returns to its *also* form (§7) — clearing the name releases the user's override on both surfaces at once.

The automation also declines to write a value that would not change. A reorder that leaves the main where it was, or a field already equal to the new main's, produces no operation at all — so a gesture's batch, and the undo step it becomes, carries only writes that alter something, and "re-derive only when the main changed" needs no rule of its own.

**A player left with no main keeps its name and loses its number.** Deleting a player's last instrument is allowed — an empty player is a legitimate transient, and the composers already create one inside every whitespace drop's own transaction — but it leaves nothing to derive an identity from. So the name and short-name stand, being the last thing written and possibly the user's own; the number is cleared, under the same comparison, because a player belonging to no kind belongs to no set, and an ordinal that distinguishes nothing is noise. This is the two→one crossing carried onto the musician axis. Dropping an instrument back in lands at index 0 and is therefore a main replacement, so the player's identity resumes from the new main: the whole exchange falls out of rules already stated.

**What the gesture creates enters unnumbered.** The freeze draws its line at the gesture's edge: it protects what existed *before* the gesture, never what the gesture itself makes. An entity a gesture creates — an instrument dropped from the library, or a cloned musician or instrument — carries no number into the pass. Where the gesture carries a **value** — a library instrument dropped into the piece — the handler strips the number as it resolves the drag, before anything is composed; it belongs there rather than in the composers because one composer serves both a library value and a source VPD, so only the handler is a place where a value is always in hand. Where the gesture works **by reference** — a clone, whose copy is made server-side and inherits the source's number — nothing local can be stripped, so the resulting projection models the copy as unnumbered and the pass's own operation overwrites the inherited value inside the same transaction. That is safe rather than lucky: a clone always leaves its kind with two or more members, so the pass always assigns the copy a number. So a cloned `Flute 1` enters the pass unnumbered and takes the next free number, rather than carrying a second `1` that the freeze could then never repair; and a library instrument a user happened to number does not smuggle that number into the piece. What existed is frozen; what is created is the pass's to number, and the invariant's "writes only to `nil`" stays literally true.
...
**A gesture can touch more than one kind** — a musician cloned with its doublings, a multi-selection deleted across kinds — and each kind speaks for itself: one notification per kind with a non-`none` signal, carrying that kind's own colour. A gesture that leaves Piccolo with a gap while making Flute whole raises two — a warning, *"Piccolo numbering has a gap."*, and a green, *"Flute numbering is complete again."* — rather than one line mixing the two, which could only be coloured by one at the other's expense, and the colour is the news. The count is bounded by the *distinct kinds* a gesture touched, never by the entities it moved: a twelve-musician delete spanning three kinds raises three notifications, not twelve, and one of twelve flutes raises a single notification. Both colours auto-dismiss; neither demands an acknowledgement.
...
**A Layout's `:name` is the one exception, and it is a name rather than a display value.** Where the automation generates it, the composed string is what is stored, numeral included — `Part: 5. Violin`, exactly what the pane header shows. That is not a rendering decision written back into the model but the consequence of a field showing what is stored: a name whose numeral existed only on screen would leave the layout's own Name field and its header disagreeing, and that discrepancy invites the user to repair it by typing the numeral in, which takes the layout out of the automation's hands for good. The stored name therefore does not chase the settings — a part named under Arabic keeps `5.` when the piece turns Roman — and the automation stands off from it thereafter, exactly as it stands off from a name the user typed (§3). A musician's or an instrument's `:number` is untouched by this: it remains an integer, and its label is composed at render as above.

### 6. Numerals: three piece settings

How a number joins its name to make a displayed label is governed by three piece settings ([ADR-0016](0016-Settings.md)), declared with `defsetting` and read over the polymorphic API like any other piece setting. They control presentation only — the stored `:number` is always a plain integer. The one place a composed numeral is *stored* is a Layout's `:name` (§5), where the settings in force when the automation writes it are fixed into the string and do not follow later changes.
...
**When the header does not name an instrument, the parenthesis lists them all.** The word *also* presumes the header already names the player's main instrument, which holds whenever the musician's name matches it. Where the user has given the player a name of their own it does not: `Multi-Clarinet Guy` names no instrument, and a performer's own name — `Erik` — names one even less, so a list of what he *also* plays would silently omit the very instrument he leads with. So the same comparison that governs re-derivation (§3) chooses the form of the annotation — where the musician's name matches its main's, the parenthesis reads *also* and lists the doublings; where it differs, the connector is dropped and the parenthesis lists **every** instrument the player holds, main included, in the same short-name-with-number form:

```
> Flute 1 (also Picc. 2)
> Erik (Cl. in E♭, Cl. in B♭, Cl. in A, Bass. hn.)
```

A player with a single instrument and a name of their own is annotated too — `> Solo Guy (Vln.)` — because without it nothing on the collapsed row would say what they play. One comparison, two consumers: it decides what a gesture may overwrite, and it decides how the header reads.

**A player holding nothing says so.** With no instruments there is neither a main to name nor doublings to list, so the parenthesis carries a marker instead: `Erik (playing no instrument)`. It is a statement about the player rather than about the row — a childless row is easy to overlook, and the parenthesis is already where this header says what the player holds. Like the connector, it is an ordinary interface string with its own `tr` key across every interface locale — and like the connector it is translated **without** the brackets: the string is `playing no instrument`, exactly as the connector's is `also`. The parentheses belong to the annotation's own form and are added once where it is rendered, so neither translation carries punctuation, and no locale can vary it.
...
**Installing a new main re-derives the musician.** Three gestures can put a different instrument at the top of a musician's list: a doubling dropped at index 0, a deletion that promotes a doubling to main, and an instruments reorder. Each re-derives that musician's name, short-name and number from the new main, under the self-limiting comparison of §3 — the automation replaces those fields only where they still match the outgoing main's. A newly created musician needs none of this: its fields are seeded from its first instrument as it is made.

**Two occasions, kept apart.** A musician's fields are written on two distinct occasions, and the design keeps them separate. The first is the mirror: the pass changed the number of an instrument that is some musician's main, and the number alone follows. The second is a main replacement: the gesture changed *which* instrument is main, and name, short-name and number are all taken from the new one. The first is intrinsic to the numbering pass and computed with it; the second is not — an instruments reorder changes no counts and no kinds, so no numbering pass runs for it at all, and yet the musician must still be re-derived. Where both apply to the same musician, the replacement supersedes the mirror.

One ordering detail matters, and the design would be wrong without it. The re-derivation reads the new main out of the projection — where a freshly created instrument is still `nil`, since what a gesture creates enters unnumbered (§3). So before it runs, the pass's own instrument assignments are applied to a local copy of that projection, and the re-derivation reads the number the batch is about to write rather than the `nil` the projection still holds. Without that step, "the replacement supersedes the mirror" would *wipe* the player's number in precisely the case where the pass had just given its new main one. Only the instrument assignments are applied, never the mirror's own musician operations, which would corrupt the comparison by pitting a number the pass had just written against the outgoing main's. It is a local prediction of the batch, computed from the source before submission — not a read of any result.
...
- **The automation respects the user, on both axes.** On the instrument axis it only ever numbers the unnumbered and clears the last survivor, so a number the user sets by hand is safe. On the musician axis, where a gesture must be free to re-derive a player's identity from a new main, the same respect is obtained by comparison rather than by refusal: the automation replaces only values that still match what it last wrote, so a name the user has changed is left alone. The protection is strong for names, whose divergence is unmistakable, and weak for the musician's number, which in the ordinary case equals its main's and is therefore indistinguishable from the automation's own output — a limit worth stating plainly, and of little consequence, since the number a user would want to correct is the instrument's. Numbering assists; it does not seize control.
...
- **The numbering helper is a new shared function**, and the composed-name function it feeds is shared between the Piece Window and the backend's paintlist generation, so both render numbers identically. Neither introduces a stored composed string of its own; both derive from the model. A Layout's `:name` is stored composed, but by the gesture that writes it rather than by either of these (§5).
...
- **A rename and a concurrent structural gesture can race, with the same character as the duplicate above.** If one collaborator renames a musician while another's gesture replaces that musician's main, the gesture's comparison runs against a projection that may not yet carry the rename, so whether the rename survives depends on ordering. The outcome is visible on the next refetch and corrected by retyping the name; no state is lost or corrupted.
