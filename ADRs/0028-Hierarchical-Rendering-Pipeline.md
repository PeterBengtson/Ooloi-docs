...
flowchart LR
    ...
    B --> B2[Atoms with Extents<br/>Min & Ideal Widths]
    ...
    E1[Min/Ideal Bounds<br/>System Width] --> E
    E --> E2[Final Positions<br/>Locked Layout]

    F1[Final Positions<br/>Locked Atoms] --> F
    F --> F2[Ties, Slurs, Beams<br/>Cross-Boundary Spanners]
...
- **Fan in**: Collects minimum and ideal widths from all measures in the stack (Stage 1 outputs)
...
- **Solution space establishment**: Defines boundaries between physical feasibility (minimum) and musical intent (ideal)
...
- **Width optimization**: Selects actual spacings between minimum and ideal bounds across the layout
- **System breaking**: Groups measures into horizontal systems based on optimized widths and available system width
...
- Measure minimum/ideal width measurements are cached and only recomputed when measure content changes
...
Stage 4 uses duration-weighted distribution to determine final layout. Each measure stack provides minimum and ideal width bounds (from Stages 1-2). Stage 4 allocates surplus in proportion to perceptual weight—longer values act as anchors; shorter values compress first—and distributes measures across systems and pages.
