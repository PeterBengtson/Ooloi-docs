...
**Where in-flight work must survive teardown, the wait belongs with whoever started it.** A subsystem that dispatches asynchronous work owns the wait for it: a piece window's close waits for that window's own refetches *as part of closing*, because the revocation it performs — which releases the piece on its last subscriber — is what invalidates them. The wait is not a step in front of the close but a step inside it, held by the resource that owns both ends: the window's hold on its piece refuses further fetches from the instant the close begins, then drains those already running, and only then revokes ([ADR-0031](0031-Frontend-Event-Driven-Architecture.md) §The piece hold). That ordering is what lets the wait be bounded without the bound deciding anything.

**A wait the owner performs still has to be joined by whoever would otherwise overtake it.** Those releases run asynchronously, and the quit's save pass deliberately does not join them one by one: the pass is already sequential, so joining per window would serialise every drain behind it for no gain. What must not overtake them is the halt, because component teardown removes the gRPC client well before the pool — so a fetch still running when the halt begins loses the client underneath it and records a failure, which [ADR-0036](0036-Collaborative-Sessions-and-Hybrid-Transport.md) §Shutdown forbids. The quit therefore waits once, after the pass and before the halt, for the condition those closes establish: that no piece is held any longer.

That is not the pool-wide drain this section rejects, and the difference is the whole point. A pool wait knows nothing about what the work needs or whose teardown invalidates it, and cannot see work a timer will submit later. Waiting for *no piece is held* names the exact obligation, names the teardown that would invalidate it, and leaves every owner still doing its own draining. The rule is not that nothing may wait before a halt; it is that a wait must know what it is waiting for.
