...
One consequence to handle rather than discover: a client's declarations and a server's piece can disagree across versions. A setting the server holds but this build does not know arrives in the projection with a value and no label, category or control — the window must decide whether to skip it or render it plainly. The converse is benign: a setting this build knows but the piece's server does not simply has no projected value, and its row is omitted.
