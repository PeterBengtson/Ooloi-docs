...
Reads resolve to `Musician` records in the layout's order (not the piece's); `copy` duplicates the *reference* only, never touching `:musicians`, the distinct physical musician minted by a following `individuate` (the `[copy, individuate]` idiom, on references). These are the api/SRV surface. Beneath them is a second, **backend-only** family — the internal accessors of the `:musician-uuids` vector itself: `get-`/`set-musician-uuid`, `get-`/`set-musician-uuids`, `add-`/`remove-musician-uuid`, declared `:vpd-category` **without `:api`**. No client ever calls a `*-musician-uuid` operation or writes `:musician-uuids` directly; the public verb maintains the vector on the caller's behalf, through the reference-maintenance family, which uses those internal accessors as its primitives. `set-musician-uuids` (internal primitive) and `set-musicians [:l n]` (public verb) are different operations at different layers, not two names for one. The internal vector is declared `:no-move :no-copy` (and, without `:has-id`, has no `reorder-musician-uuid`): not a claim that a layout cannot be moved-within or copied-into — it plainly can, through the public verbs — but only that the redirects maintain the vector through whole-vector `set-musician-uuids`, so per-item mover/copier primitives on the internal vector are never needed.
