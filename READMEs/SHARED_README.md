...
```
shared/
├── cljfx/                            ; Local cljfx node override (not on a source path)
├── dev/                              ; Development helpers
│   ├── bench/                        ; Benchmark suite
│   ├── repl.clj                      ; REPL helpers
│   └── user.clj                      ; REPL user namespace
├── resources/                        ; Application resources
│   ├── app-settings/                 ; Generated frontend settings defaults
│   ├── css/                          ; Stylesheets
│   ├── i18n/                         ; Translation catalogues (PO files)
│   ├── icons/                        ; Platform-specific icons (icns, ico, png)
│   ├── music-text-fonts/             ; Text fonts for musical documents
│   ├── smufl-fonts/                  ; SMuFL notation fonts
│   └── splash/                       ; Splash screen assets
├── src/app/clojure/ooloi/shared/     ; **Combined Application**
│   ├── app.clj                       ; Application entry point
│   ├── system.clj                    ; combined-config and action-handler wiring
│   ├── splash.clj                    ; Splash screen
│   └── javafx_init.clj               ; JavaFX toolkit initialisation
...
│       ├── api.clj                   ; Unified API entry point for all shared contracts
│       ├── async.clj                 ; Waiting for a condition an atom is about to satisfy
│       ├── build.clj                 ; Build utilities and packaging
│       ├── collaboration.clj         ; Read helpers over the combined-app system map
│       ├── config.clj                ; CLI argument, environment and default resolution
...
│       ├── srv_client.clj            ; Dynamic var holding the current gRPC client
...
│       │   └── remembered_alterations.clj ; When accidentals should be printed
...
│       │   ├── service_builder.clj   ; ServerServiceDefinition builder
│       │   ├── transport.clj         ; In-process and network marshallers
│       │   ├── tls.clj               ; TLS certificate management
│       │   ├── headers.clj           ; gRPC metadata constants and context
│       │   └── errors.clj            ; Connection-error classification
│       ├── i18n/                     ; Localisation
│       │   ├── tr.clj                ; Localisation infrastructure
│       │   └── verify.clj            ; Build-time completeness verification
│       ├── models/                   ; **Complete Data Model**
│       │   ├── core.clj              ; Model core, macro system and system assembly
...
│       │   ├── event_subscription.clj ; Subscriptions for collaborative notifications
...
├── test/                             ; Test suite (three source roots)
│   ├── app/clojure/                  ; Combined application integration tests
│   ├── clojure/                      ; Shared business logic tests
│   └── util/                         ; Test infrastructure macros
...
- **`grpc/clojure_conversion.clj`**: Context-free Clojure ↔ internal map conversion
- **`grpc/protobuf_bridge.clj`**: Internal map ↔ Java protobuf objects
