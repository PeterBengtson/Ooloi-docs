...
**The `:transport` option configures both halves.** `{:transport :network}` sets the backend gRPC server's transport and points the frontend `grpc-clients` at that server — the same transport, and the host and port it binds, taken from the effective config so an `:extra-config` port override reaches the client too. Under the default `:in-process`, the client takes its `backend-server-name` from the server component it refs (`grpc_clients.clj`, `init-key`). The remedy above therefore applies in both modes: `register-with-server` on the application's own client reaches the server the macro started, whichever transport it was asked for.
