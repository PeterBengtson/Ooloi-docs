...
A window enters the record **only on gaining focus**, never on being registered, so it records genuine focus history rather than an order nobody chose. One consequence is worth stating because it is not obvious: a window that declares it must not take focus when opened — the ambient indicators of [ADR-0042](../ADRs/0042-UI-Specification-Format.md) — is deliberately excluded, because JavaFX offers no way to show a Stage without activating it, so such a window really does take focus briefly and hand it back. Left in the record it would sit directly behind whatever regained focus, and the next close would raise it. The exclusion keys on the window's own declaration rather than on any category, so a floating palette that *wants* focus simply omits the declaration and behaves like any other window. See [UI Architecture — The Focus-Order Record](../research/UI_ARCHITECTURE.md).

The Window menu's **open-window list** is another consumer. Below its static commands the menu carries a live entry for every open piece window — labelled by the window's title (with its `●` dirty / `⇄` shared decorators), sorted alphabetically, and with the foremost window (the `:active-window-id` one) checkmarked; selecting an entry raises and focuses that window. A UI-Manager subscriber on `:window-lifecycle` keeps the list current — an open adds an entry, a close removes it, a focus change moves the checkmark — and the same title sink that retitles a window re-derives its menu label, so a rename or a decorator change shows on the menu at once. Because this list reads only the manager's own window registry and active-window state — never the live combined-app system map — its subscriber lives in the UI Manager itself, unlike the enablement refresh, which is wired in `start-app!` where the system-state provider is. See [UI Architecture — The Window menu's open-window list](../research/UI_ARCHITECTURE.md).
