...
- **Selection is one flat map** — a level (`:musician` / `:instrument` / `:staff`, and `:layout` in the Layouts pane) tagged onto a single anchor-first id-vector. Mutual exclusivity between levels is structural: there is only ever one selection. The type-agnostic selection primitives (plain / toggle / extend / arrow-move) operate on that vector, each caller supplying its level's ordered sibling ids; the render derives every pane's highlight from the map, so multi-selection needs no extra rendering.
...
- **A drag-and-drop gesture composes one backend transaction.** A drop reads what was dragged and where it landed, composes the corresponding backend calls, submits them as one atomic batch, and trusts the invalidation/refetch cycle to redraw — never an optimistic local mutation. Every *copy* is an ordinary insert followed by a re-identify step in the same transaction, so the copy stops sharing identity with its source before anything observes it.
...
The Layouts pane is built by **replicating** this realisation — the same dispatcher, the same flat selection and expansion maps at a new level, the same drop-composes-a-transaction shape — rather than by sharing a common abstraction. Extracting a shared "selectable, expandable tree pane" is deliberately deferred until that third consumer exists and the shape has stopped moving: replicate now, extract later. The Piece window as a whole is specified in [ADR-0053](../ADRs/0053-Piece-Window-and-Piece-Preferences.md).
