...
**The one exception is `javafx.scene.robot.Robot`.** A Robot click on a *shown* Stage generates real toolkit input and grants genuine OS focus — the only mechanism that does. Behaviour that reads live focus is therefore tested with Robot, not `requestFocus`. The canonical case is `ui-manager/active-piece-id`, which resolves the piece the user is acting on from the **focused** window (a piece window's `:window/id` is its piece-id; a layout window's `:window/piece-id` names its parent piece), returning `nil` when no piece window holds focus — the query the File-menu piece actions (Save / Save As / Close) and their enablement target. Its test shows a real piece window and a real non-piece window and clicks each with the shared Robot helpers (`util.robot`), asserting `active-piece-id` follows focus.
