...
The Window menu's **open-window list** is another consumer. Below its static commands the menu carries a live entry for every open piece window **and for every window those pieces own** — labelled by the window's title (with its `●` dirty / `⇄` shared decorators), sorted alphabetically, and with the foremost window (the `:active-window-id` one) checkmarked; selecting an entry raises and focuses that window. Membership is therefore a question about two keys on the registry entry — `:window/type :piece`, or a `:window/piece-id` naming one — rather than a query by type alone, and an owned window is listed under its own title rather than its piece's. The list is flat, but a piece and the windows it owns are grouped together by dividers; [ADR-0053](../ADRs/0053-Piece-Window-and-Piece-Preferences.md) §5 specifies the grouping rules. A UI-Manager subscriber on `:window-lifecycle` keeps the list current — an open adds an entry, a close removes it, a focus change moves the checkmark — and the same title sink that retitles a window re-derives its menu label, so a rename or a decorator change shows on the menu at once. Because this list reads only the manager's own window registry and active-window state — never the live combined-app system map — its subscriber lives in the UI Manager itself, unlike the enablement refresh, which is wired in `start-app!` where the system-state provider is.
...
### 3.5 Three Layers of Remembered State

Ooloi keeps three kinds of remembered state in separate files, because they are not the same kind of fact — and only two of them belong to a window at all.

The first is **per-window**: where the window sat, how big it was, and whatever else its kind keeps — a piece window's two pane-expansion maps and its split-divider proportion. This is written continuously as you work — a quarter-second after you stop dragging or resizing, the moment you toggle a pane open or shut, and immediately when a window closes. It answers *how was this window arranged*.
...
The third is **per-surface**, and it is not a window's at all. How a settings surface is arranged — which of its tiles are open, and which tab it is showing — is written under a named directory, one per surface: the App Settings window's, the Instrument Library's, and one shared by every Piece Preferences window. Each fact keeps its own small file, a map of setting to open-or-closed beside a single category, so a reader of either needs no knowledge of the other's shape and a map already on disk stays readable when the other fact is added beside it. Keying it by surface rather than by window is the whole point: opening the accidentals tile once leaves it open for every piece opened afterwards, because that is a fact about how this person likes to read settings and not about any score. A scope per piece would start every new piece closed on the first tab, so the preference could never stick, and would accumulate one directory per piece ever opened with nothing to remove them. What a window keeps here it reads **once, as it opens**, and holds in its state thereafter — its content is a pure function of that state, and a read on the render path would be a file read per row per render pass.
...
| `ooloi-checkbox` | `:text-key` + `:locale` (guarded), `:error?` | `:check-box`; the one-method boolean control. Resolves `:text-key` via `tr`; omits `:text` when none given (label via `ooloi-labelled-field`); `:selected`/`:on-selected-changed` pass through. `:error?` applies `styles/error-style` to the control, which recolours the `.box` beneath it — the control paints no background of its own |
...
| `ooloi-dense-combo-box` | `:choices` (optional), `:locale` (optional cache-buster), `:error?` (in the `:choices` arm, which every call site takes) | Dense `:combo-box` with AtlantaFX base style-classes and ordered lifecycle (items-before-value); with `:choices`, uses cljfx cell factory pattern — items stay as keywords, `:fx/event` delivers keywords; `:locale` forces re-render so cell factories capture current `tr` |
...
| `ooloi-dense-text-field` | `:on-commit` (optional), `:error?` | Dense `:text-field` with AtlantaFX base style-classes. With `:on-commit`: fires `(on-commit text-string)` on Enter and focus loss. Without: all props pass through. `:error?` applies `styles/error-style` |
| `ooloi-dense-numeric-field` | `:on-commit` (optional), `:error?` (inherited, since it delegates) | `ooloi-dense-text-field` at six columns with `:min-width :use-pref-size`. The floor matters: a preferred width is only a hint, so without it a compressed row shrinks the field to its padding and the value disappears. The one way to describe a numeric input, and the place locale-dependent number formatting would go (values are invariant today) |
| `ooloi-dense-spinner` | `:min`, `:max`, `:value`, `:error?` | Dense `:spinner` with integer value factory; non-editable by default (small numeric ranges). `:error?` applies `styles/error-style` to the base map both arms build from, so it reaches the control whether or not an `:on-commit` wraps it |
| `ooloi-list-cell` | `:text-key` + `:locale`, or `:raw-text`; `:leading-icon`, `:trailing-icon`, `:selected?` | A `:list-view` row's describe map on the `"list-cell"` base class — the one way to describe a row, with no raw `:list-cell` describe in application code. Either icon puts the row into a graphic-only HBox: the leading `FontIcon` before the label, the trailing one pushed right by a spacer. A `:selected?` row carries `styles/selected-style`, the app-wide selection colour |
| `ooloi-dense-list-view` | (none; all props pass through) | Dense `:list-view` carrying the `"list-view"` base style class — the one way to describe a list view. The base class is not optional: cljfx's `:style-class` calls `setAll()`, so omitting it strips the class AtlantaFX's rendering is anchored to |
...
| `ooloi-labelled-field` | HBox label + control; with optional `:description`, label + VBox[control, muted wrapped description] (tri-partite). Control-agnostic; reusable in editors and dialogs. `:error?` is a fact about the field rather than the widget in it — the argument the description already makes — so it is accepted here and passed to the `:control`, which knows how to show it. The label is not restyled: what is wrong is the value, and the label only names the field |
...
| `ooloi-layout-editor` | Layout editor built on `ooloi-openable-pane` — the Layouts pane's peer of `ooloi-musician-editor`, built to the same pattern so a reader who has understood one entity editor finds nothing new. `:graphic` the composed layout title with its doubling annotation; content the layout's own field rows followed by the musicians it lists, each an openable-but-**empty** pane (the Layouts pane shows musicians, never their staves). `:name` is the layout's only editable field, and blanking it returns the header to its derived form — a name *override* rather than a stored title. Selection is context-scoped to `[:layout id]`, so the same musician selected in the Musicians pane or another layout does not light up here. **Requires `:dispatch`**. Accepts `:layout`, `:musicians`, `:numeral-settings`, `:selection`, `:expanded`, `:locale`, and the window-local drag callbacks `:on-layout-drag-detected`, `:on-layout-musician-drag-detected`, `:on-drag-over` |
...
| Reorder within a parent — drag (`reorder-<elem>`, shared drop-target scan) | ✓ — instrument level: own scan | ✓ | ✓ |
...
- **Expansion is one flat map, and every level rests closed until the user opens it** — musicians, instruments and staves in the Musicians pane, layouts and their musicians in the Layouts pane. The map is keyed by each entity's stable structural id, so a toggle survives the projection refetch, and it holds only the ids the user has touched rather than being seeded with every id on each refetch. Why a container does not open merely because it has just been created — and why a closed row still says what it holds — is [ADR-0053](../ADRs/0053-Piece-Window-and-Piece-Preferences.md) §3. That same map of explicit toggles is what the window **remembers across sessions**: on close, both panes' expansion maps and the split-divider position are written to the window's own persisted record (keyed by the piece's UUID, alongside its geometry); on reopen they are restored, so a piece comes back arranged as it was left, while a pane the user never touched rests closed. It is a *map* rather than a set because an entry records a toggle in either direction and the whole map is persisted and restored as it stands.
...
**A test and a quit end differently.** A test halts *abruptly*, from outside, at whatever instant its body happens to finish: startup work may still be in flight, and nothing has been given the chance to conclude. That is precisely what `util.client/halt-app!`'s wait is for. A quit does not end that way. It runs the save pass first, resolving and closing each piece window in turn, and every subsystem that starts asynchronous work is responsible for waiting on its own. A piece window's close waits for that window's own refetches *within* the close rather than ahead of it: releasing its hold on the piece refuses further fetches, drains those already running, and only then revokes the subscription that releasing the piece depends on ([ADR-0031](../ADRs/0031-Frontend-Event-Driven-Architecture.md) §The piece hold).

Those releases run on the pool, and the pass does not join them window by window — it is already sequential, so that would serialise every drain behind it for nothing. The **halt** is what must not overtake them: Integrant removes the gRPC client well before the pool, so a fetch still running when the halt begins loses its client and records a failure, and quitting must produce no failure record in any deployment shape ([ADR-0036](../ADRs/0036-Collaborative-Sessions-and-Hybrid-Transport.md) §Shutdown). So the quit waits once, between the pass and the halt, until no piece is held. That is not the pool-wide drain this section rejects — it names the obligation it waits for and the teardown that would invalidate it, which a pool wait cannot.
...
It is driven by a uniform validation closure: `(fn [value] → nil | failure)`, where a failure is **data rather than a message** — a map naming a `:failure` type and carrying what that type needs to be described. The closure wraps whatever validation backend applies — the app settings registry, the piece settings registry, or a `clojure.spec` check on a domain field, such as an instrument's number or the pitch pair one half of a range would complete. What the spec backend is handed is always a value and never an entity built in order to be validated, because an editor is mounted over the structural projection as readily as over records. The form field formatter calls it without knowing what is behind it, so a window gets the restore and the rejection signal by passing `:validate`, and the sentence by passing `:on-reject` beside it ([ADR-0042](../ADRs/0042-UI-Specification-Format.md) §Editor Field Commits).
