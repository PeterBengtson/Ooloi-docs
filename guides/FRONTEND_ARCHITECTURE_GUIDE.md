...
The coverage therefore lives where work is *dispatched*, not where it is called, so that nothing at a call site has to remember to catch. Three mechanisms feed it. The shared thread pool recovers any uncaught exception from a completed task and hands it to a per-pool error handler. Every renderer is created with an explicit `:error-handler`, which is the only thing that can see a render failure — cljfx catches those internally and never lets them escape. And an uncaught-exception handler is attached to the JavaFX thread itself, for everything on that thread which never passes through a renderer at all: `fx/run-later!` bodies, event handlers, animation callbacks.

All three arrive at one function, so a failure looks the same to the user whichever thread it happened on. In the combined application it becomes an error notification and reaches the user instead of vanishing; a standalone backend, having no UI, keeps a handler that only logs — and has no JavaFX thread to cover in the first place.
